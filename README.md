...
1. 트랜잭션 
  ...
  - 주문이 취소되어도 직원이 접수하여 음료제조를 시작한 주문인 경우 주문 취소는 원복되어야 한다. Saga(보상 트랜잭션)
...
*****
결제가 이루어진 후에 카페로 이를 알려주는 행위는 동기식이 아니라 비 동기식으로 처리하여 카페 시스템 처리를 위하여 결제주문이 블로킹 되지 않아도록 처리한다.
